...
App.initFilters = function () {
  ...
  var toggleEl      = document.getElementById('filter-solo-acciones');
  ...
  function onChange() {
    ...
    App.state.filters.soloAcciones  = toggleEl ? toggleEl.checked : false; // #1
  ...
  if (toggleEl)   toggleEl.addEventListener('change',   onChange); // #1
};

App.applyFilters = function (orders, criteria) {
  return orders.filter(function (o) {
    if (criteria.soloAcciones  && !o.action)                                        return false; // #1
